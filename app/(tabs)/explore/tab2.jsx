...
const Tab2Screen = () => {
  ...
  const { user } = useAuth();
  const [updatedPosts, setUpdatedPosts] = useState(sortedPostsByLike); // Trạng thái bài viết được cập nhật

  useEffect(() => {
    setUpdatedPosts(sortedPostsByLike); // Cập nhật lại posts khi có dữ liệu mới
  }, [sortedPostsByLike]);
