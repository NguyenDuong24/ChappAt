...
const Tab1Screen = ({ isActive }) => {
    ...
    // IMPORTANT: WE NO LONGER RETURN NULL OR BLANK VIEW IF INACTIVE.
    // This preserves the FlatList state and data.
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background, display: canRender ? 'flex' : 'none' }]}>
            <Animated.FlatList
                ...
                refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => refresh && refresh('latest')} tintColor={colors.tint} colors={[colors.tint]} />}
                onScroll={onScroll}
                scrollEventThrottle={16}
                showsVerticalScrollIndicator={false}
                onEndReachedThreshold={0.5}
                onEndReached={() => { if (hasMore && !loadingLatest && loadMore) loadMore(); }}
