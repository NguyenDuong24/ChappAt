...
// Reusable Shimmer Component for Premium Feel
...
// Memoized Loading Component (Full Page Skeleton)
...
const Tab1Screen = () => {
  ...
  const { loadMore, deletePost, toggleLike, addComment, refresh, updatePostPrivacy } = useExploreActions() || {};
  ...
  const onRefresh = useCallback(async () => {
    if (refresh) await refresh('latest');
  }, [refresh]);
  ...
  }, [loadingLatest, loadingInitial, hasMore, currentThemeColors]);
  ...
  if (loadingInitial && latestPosts.length === 0) {
    return (
      <LoadingView
        backgroundColor={currentThemeColors.background}
...
export default memo(Tab1Screen);