...
const Tab2Screen = ({ isActive }) => {
    ...
    if (!canRender || (loadingInitial && trendingPosts.length === 0)) {
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background, display: canRender ? 'flex' : 'none' }]}>
            <Animated.FlatList
                ...
                refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => refresh && refresh('trending')} tintColor={colors.tint} colors={[colors.tint]} />}
                onScroll={onScroll}
                scrollEventThrottle={16}
                showsVerticalScrollIndicator={false}
                onEndReachedThreshold={0.5}
                onEndReached={() => { if (hasMore && !loadingTrending && loadMoreTrending) loadMoreTrending(); }}
